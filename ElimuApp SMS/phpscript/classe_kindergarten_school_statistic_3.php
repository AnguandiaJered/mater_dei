...
class kindergarten_statistic_3 extends form_generator 
{
	...
	function table_one_3($conn,$division_id)
	{
	$this->table_header='
<table width="100%">
  <tr bgcolor="#E2E9FE">
  ...
  return $this->table_header.$this->table_content.$this->table_total.$this->table_footer;
	...
	function get_sub_regime($conn,$type,$sous_division_id)
	{
		...
		while($this->row=mysql_fetch_array($this->q))
		{
			...
			$this->total_g+=+($this->get_classe_number($conn,$this->loop_ids,'tbl__23tableau__i__pre__primaire',5,'identitification__pre__primaire_id')+$this->get_classe_number($conn,$this->loop_ids,'tbl__23tableau__i__pre__primaire',6,'identitification__pre__primaire_id'));//on addition GF par sous division
			$this->total_f+=+$this->get_classe_number($conn,$this->loop_ids,'tbl__23tableau__i__pre__primaire',6,'identitification__pre__primaire_id');//on addition F par Sous division
			//content section starts
			$this->content.='<td>'.($this->get_classe_number($conn,$this->loop_ids,'tbl__23tableau__i__pre__primaire',5,'identitification__pre__primaire_id')+$this->get_classe_number($conn,$this->loop_ids,'tbl__23tableau__i__pre__primaire',6,'identitification__pre__primaire_id')).'</td>
			<td>'.$this->get_classe_number($conn,$this->loop_ids,'tbl__23tableau__i__pre__primaire',6,'identitification__pre__primaire_id').'</td>';	
			}else
			{
	...
	function generate_total_loop($conn,$sub_regime_id,$division_id,$type)
	{
		...
		if($type==1)
		{
		$this->total_looping+=+$this->get_classe_number($conn,$this->loop_ids,'tbl__23tableau__i__pre__primaire',5,'identitification__pre__primaire_id');//garcons
		}else if($type==2)
		{
			$this->total_looping+=+$this->get_classe_number($conn,$this->loop_ids,'tbl__23tableau__i__pre__primaire',6,'identitification__pre__primaire_id');//filles
		}
