...
function foreign_value($ref,$table,$fld_range,$database)
{
	$j=0;
		//pk find start
	//echo "show columns  from ".$table." ORDER BY date desc <br>";
...
function display_fees($db,$tbl,$ref,$explode_string,$output,$student_id)
{
	...
	while($row=mysql_fetch_array($q))
	{
		...
		if(sum_rowsone($db,'tbl__27payer__les__frais','montant','where inscription__des__eleves_id="'.$student_id.'" and frais__scolaires_id="'.$row[0].'"') ==$row['montant']){$fee='<font color="#009900">'.$row[$output].'</font>';}else{$fee='<label>'.$row[$output].'</label><input type="checkbox" onclick=" fee_'.$j.'()">';}
		$content.='<div style="float:left; width:200px;">
		<div id="show_pay_id_'.$j.'">'.$fee.'
...
function getvalues($db,$date,$feeid,$type,$classeid)
{
	...
	while($read=mysql_fetch_array($k))
	{
		...
		$content.=foreign_value('where inscription__des__eleves_id="'.$read['inscription__des__eleves_id'].'"','tbl__17inscription__des__eleves','nom__complet',$db).'|&nbsp;Montant: '.$read['montant'].'| '.foreign_value('where classe_id="'.$el_classe_id.'"','tbl__15classe','nom__de__la__classe',$db).'<br/>';
		}
	}
	if($type=='txt')
		{
		...
		while($read=mysql_fetch_array($k))
		{
			...
			if($feeid!='all'){if($read[0]!=$feeid){continue;}}
				...
				else if($type=='body')
				{
					...
					$fees=foreign_value('where classe_id="'.$classeid.'"',' tbl__15classe','frais__scolaires_id',$db);
					if(compare_from_exp($fees,$read[0])<1){$insolvabilite='pas appliqué';}
					else{
				...
				else if($type =='total_payer')
				{
					...
					{
						$content+=+sum_rowsone($db,'tbl__27payer__les__frais','montant','where frais__scolaires_id="'.$read[0].'" and inscription__des__eleves_id ="'.$stdid.'" order by date asc');
					}
...
<script>
$(document).ready(function() {
    var tab2 = $("#map-table-sortie").dataTable({
		        "bFilter": false,
		         "bPaginate": false,
		         "bInfo":false,
		         "bStateSave": true
...
<table class="table table-striped table-bordered table-hover table-green" id="map-table-sortie">
<thead>
<tr bgcolor="#FFFFDD">
...
{
	//echo "$from and $to<br>";
$q=$db=mysql_query("select *,DATE_FORMAT(date, '%d/%m/%Y') AS date from tbl__29sortie where date between '".$from."' and '".$to."' order by date asc") or die('Query Failed'.mysql_error());
}
else if($sel == 2)
{
	//echo "$from and $to<br>";
$q=$db=mysql_query("select *,DATE_FORMAT(date, '%d/%m/%Y') AS date from tbl__29sortie where categorie_id=2  and caisse__ou__banque_id ='".$caisse_bank."' and  date between '".$from."' and '".$to."' order by date asc") or die('Query Failed'.mysql_error());	
}
else if($sel == 1)
{
	//echo "--$from and $to<br>";
$q=$db=mysql_query("select *,DATE_FORMAT(date, '%d/%m/%Y') AS date from tbl__29sortie where categorie_id=1  and caisse__ou__banque_id ='".$caisse_bank."' and  date between '".$from."' and '".$to."' order by date asc") or die('Query Failed'.mysql_error());	
//--2015-09-01 and 2017-09-16

/*$q=$db=mysql_query("select *,DATE_FORMAT(date, '%d/%m/%Y') AS date from tbl__29sortie where categorie_id=1  and caisse__ou__banque_id ='".$caisse_bank."' and  date between '2015-09-01' and '2017-09-16' order by date asc") or die('Query Failed'.mysql_error());*/	
...
function fee_incoming($db,$from,$to,$type='data',$sel,$caisse_bank)
{
	...
	if($sel == 3)
	{
		$q=$db=mysql_query("select * from  tbl__27payer__les__frais where  date between '".$from."' and '".$to."'order by date asc") or die('Query Failed'.mysql_error());
	}
	else if($sel == 2)
	{
		$q=$db=mysql_query("select * from  tbl__27payer__les__frais where  	treasure_bank =2 and treasure_bank_id='".$caisse_bank."' and  date between '".$from."' and '".$to."'order by date asc") or die('Query Failed'.mysql_error());
	}
	else if($sel == 1)
	{
		$q=$db=mysql_query("select * from  tbl__27payer__les__frais where  	treasure_bank =1 and treasure_bank_id='".$caisse_bank."' and  date between '".$from."' and '".$to."'order by date asc") or die('Query Failed'.mysql_error());
	}
	$body_arr=array();
	$total=0;
	while($row=mysql_fetch_array($q))
...
{
//echo $sel."<br>".$caisse_bank."<br>";
	//mise a jour guyL
	$accountid = $caisse_bank;
$categorieid = $sel;

echo $to."<br>";

$incoming=(sum_rowsone($db,'tbl__33entree','montant','where categorie_id="'.$categorieid.'" and caisse__ou__banque_id="'.$accountid.'" and date <= "'.$to.'"')+sum_rowsone($db,'tbl__27payer__les__frais','montant','where treasure_bank="'.$categorieid.'" and  treasure_bank_id ="'.$accountid.'" and date <= "'.$to.'"'));

$outcoming=sum_rowsone($db,'tbl__29sortie','montant','where categorie_id="'.$categorieid.'" and caisse__ou__banque_id="'.$accountid.'" and date <= "'.$to.'"');
$available_solde=($incoming-$outcoming);

$solde = $available_solde;

if ($sel == 3) {
	$incoming=(sum_rowsone($db,'tbl__33entree','montant','where date <= "'.$to.'"')+sum_rowsone($db,'tbl__27payer__les__frais','montant','where 1'));

	$outcoming=sum_rowsone($db,'tbl__29sortie','montant','where date <= "'.$to.'"');
	$available_solde=($incoming-$outcoming);
...
<script>
$(document).ready(function() {
    var tab1 = $("#map-table-example").dataTable({
		        "bFilter": false,
		         "bPaginate": false,
		         "bInfo":false
		       } );
    ...
    <table class="table table-striped table-bordered table-hover table-green col-sm-12" id="map-table-example">
...
$total_footer='<tfoot><tr bgcolor="#FFFFDD">
<td colspan="3"><strong>Total</strong></td>
...
//'.($total-sortie($from,$to,$db,'total',$sel,$caisse_bank,$design)).'
switch($sel)
{
	case 3:
	//echo $sel."<br>";
...
$periode='<h2 class="_8nov17_titre1"> Du: '.french_date($from).' Au: '.french_date($to).'<br>';
$head_title='<div style="width:100%; margin-bottom:20px;">'.$periode.$des.'</div></h2>';
if($design!='html'){$attribut1=' width:100%;'; $attribut2=' width:100%;';$title=''; }else{$attribut1=''; $attribut2='';$title=$head_title;}
$part_a='<div style="width:100%"><div style="float:left; '.$attribut1.' "  class=" col-sm-6">'.$header.$content.$total_footer.$footer.'</div>';
$part_b='<div style="float:left; '.$attribut2.' "  class=" col-sm-6">'.sortie($from,$to,$db,'data',$sel,$caisse_bank,$design).'</div></div>';
 ...
 function read_row_jc($db,$string_v,$type)
{
	...
	//echo 'select * from tbl__33entree where entree_id="'.$exp[1].'" order by date asc <br>';
	if($exp[2]=='e')
	{
	 if($type==1)
	 {
	  return french_date(foreign_value('where entree_id="'.$exp[1].'" order by date asc','tbl__33entree','date',$db));//date
	 }
	 else if($type==2)
	 {
	  return foreign_value('where entree_id="'.$exp[1].'"order by date asc','tbl__33entree','motif',$db);//libelle
	 }
	  else if($type==3)
	 {
	  return foreign_value('where entree_id="'.$exp[1].'"order by date asc','tbl__33entree','montant',$db);//libelle
	 }
	 else if($type==4)
	 {
		 $bc=foreign_value('where entree_id="'.$exp[1].'"order by date asc','tbl__33entree','categorie_id',$db);
		 $bcid=foreign_value('where entree_id="'.$exp[1].'order by date asc"','tbl__33entree','caisse__ou__banque_id',$db);
	  if($bc== 1)
	  {
	...
	{
		$fee_id=foreign_value('where payer__les__frais_id="'.$exp[1].'"/*order by date asc*/','tbl__27payer__les__frais','frais__scolaires_id',$db);
		$std_id=foreign_value('where payer__les__frais_id="'.$exp[1].'"/*order by date asc*/','tbl__27payer__les__frais','inscription__des__eleves_id',$db);
	...
	if($type==1)
	 {
	  return french_date(foreign_value('where payer__les__frais_id="'.$exp[1].'" /*order by date asc*/','tbl__27payer__les__frais','date',$db));//date
	 }
	 else if($type==2)
	 {
	 ...
	 {
	  return foreign_value('where payer__les__frais_id="'.$exp[1].'" /*order by date asc*/',' tbl__27payer__les__frais','montant',$db);//libelle
	 }
	 else if($type==4)
	 {
		 $bc=foreign_value('where payer__les__frais_id="'.$exp[1].'" /*order by date asc*/',' tbl__27payer__les__frais','treasure_bank',$db);
		 $bcid=foreign_value('where payer__les__frais_id="'.$exp[1].'" /*order by date asc*/',' tbl__27payer__les__frais','treasure_bank_id',$db);
	  if($bc== 1)
	  {
		return foreign_value('where  caisse_id="'.$bcid.'"','tbl__23caisse','caisse',$db);
	  }
...
$title=strtoupper('<span class="_8nov17_titre">Section: '.$mysection.', option: '.$myoption.', classe: '.$myclasse.'</span>');
/*$include = '
<link href="css/plugins/datatables/datatables.css" rel="stylesheet">
<script src="ajax/jquery-1.9.0.min.js"></script>
<script src="js/plugins/datatables/jquery.dataTables.js"></script>
    <script src="js/plugins/datatables/datatables-bs3.js"></script>
';*/
$header='<table class="table table-striped table-bordered table-hover table-green" id="map-table-example1" >
 <thead>
...
$content='';
$j=0;
$q=$db=mysql_query("select * from  tbl__17inscription__des__eleves where  	classe_id='".$classe."' order by nom__complet") or die("Query Failed".mysql_error());
while($row=mysql_fetch_array($q))
{
	$j++;
	$content.='<tr bgcolor="#FFFFFF">
...
<table width="100%" border="1">
  ...
  <tr bgcolor="#D6FED8">
   ...
   '.ShowPeriode('trim_header',$student_id,0,$optionid,$db).'
    <td colspan="2">TOTAL</td>
  </tr>'.ShowPeriodeAndmax($db,$optionid,'max_header',0,$student_id,0).ShowActivities($db,$optionid,$student_id).'
  <tr bgcolor="#00CC00"><td>TOTAL GENERAL</td>'.ShowPeriodeAndmax($db,$optionid,'total_gen',0,$student_id,0).'</tr>';
$footer='</table>';
return $header.$footer;
}

function ShowPeriode($type,$studentid,$brancheid,$optionid,$db)
{
	$trim=foreign_value('where option_id="'.$optionid.'"',' tbl__41periodicite','trimestre',$db);
	$content='';
	for($i=1; $i<=$trim; $i++)
	{
		if($i==1)
	  ...
	  $content.=' <td colspan="'.$n.'">'.$i.'e Trimestre</td>';	
	}
	if($type =='trim_header')
	{
		return $content;
...
function ShowPeriodeAndmax($db,$optionid,$type,$brancheid,$stdid,$activiteid)
{
	$header='<tr>';
	$footer='</tr>';
	$content='';
	$max_header='<td>Max</td><td>Pts Ob</td>';
	$j=0;
	$total_maxp=0;
	$periode=foreign_value('where option_id="'.$optionid.'"',' tbl__41periodicite','periode',$db);
	for($i=1; $i<=$periode; $i++)
	{
		$j++;
		if($j == 1 && $i==1)
		{
		$maxes='';
		//max periode section starts  loop level 1
		  if($type =='max_header')
		  {
			   $max_p='<td>Max Per</td>';
		  }
		  ...
		  {
			   $max_p='<td style="color:blue; font-weight:bold;">'.foreign_value('where branche_id="'.$brancheid.'"',' tbl__43branches','max__periode',$db).'</td>';
		  }
		  
		   else if($type =='sub_total')
		  {
			   
			   $max_p='<td style="color:blue; font-weight:bold;"><strong>'.SubTotal($db,$activiteid,$optionid,$stdid,$i,'max_periode').'</strong></td>';//sub total max periode
		  }
		  
		   else if($type =='total_gen')
		  {
			   
			   $max_p='<td><strong>'.TotalGen($db,$stdid,$optionid,$i,'total_gen_max_periode').'</strong></td>';//sub total max periode
		  }
		  
		//	max periode section ends loop level 1
		}
		else 
		
		//*****************************2EM TRIME STRRE STARTS*****************************************************************************
		if($j==3)
		{
			$j=0;
			 if($type =='max_header')
		     {
			$maxes='<td>Max Ex</td><td>Pt Ob</td> <td>Max Tri</td><td>Pt Ob</td>';
			 }
			  else if($type =='mark')
		  	 {
				 ...
				 if($i<5)
				 {
				$point_exam=foreign_value('where cours_id ="'.$brancheid.'" and order_id=2 and affectation_id =2 and inscription__des__eleves_id ="'.$stdid.'"','tbl__37point_eleve','mark',$db);
				 $point_periode3=foreign_value('where cours_id ="'.$brancheid.'" and order_id=3  and affectation_id=1 and inscription__des__eleves_id ="'.$stdid.'"','tbl__37point_eleve','mark',$db);
				 $point_periode4=foreign_value('where cours_id ="'.$brancheid.'" and order_id=4  and affectation_id=1 and inscription__des__eleves_id ="'.$stdid.'"','tbl__37point_eleve','mark',$db);
				$maxes='<td style="color:blue; font-weight:bold;">'.foreign_value('where branche_id="'.$brancheid.'"',' tbl__43branches','max__exam',$db).'</td><td>'.$point_exam.'</td><td style="color:blue; font-weight:bold;">'.(foreign_value('where branche_id="'.$brancheid.'"',' tbl__43branches','max__exam',$db)*2).'</td><td>'.($point_exam+$point_periode3+$point_periode4).'</td>';	
				 }
				 else
				 {
					 //OTHER PERIODE THRID TRIM ++
					 $point_exam=foreign_value('where cours_id ="'.$brancheid.'" and order_id=3 and affectation_id =2 and inscription__des__eleves_id ="'.$stdid.'"','tbl__37point_eleve','mark',$db);
				 $point_periode5=foreign_value('where cours_id ="'.$brancheid.'" and order_id=5  and affectation_id=1 and inscription__des__eleves_id ="'.$stdid.'"','tbl__37point_eleve','mark',$db);
				 $point_periode6=foreign_value('where cours_id ="'.$brancheid.'" and order_id=6  and affectation_id=1 and inscription__des__eleves_id ="'.$stdid.'"','tbl__37point_eleve','mark',$db);
					$maxes='<td style="color:blue; font-weight:bold;">'.foreign_value('where branche_id="'.$brancheid.'"',' tbl__43branches','max__exam',$db).'</td><td>'.$point_exam.'</td><td style="color:blue; font-weight:bold;">'.(foreign_value('where branche_id="'.$brancheid.'"',' tbl__43branches','max__exam',$db)*2).'</td><td>'.($point_exam+$point_periode5+$point_periode6).'</td>'; 
				 }
		 	 }
			  else if($type =='sub_total')
		  	{
				if($i<5)
				{
				$maxes='<td style="color:blue; font-weight:bold;">'.SubTotal($db,$activiteid,$optionid,$stdid,$i,'max_exam').'</td><td><strong>'.SubTotal($db,$activiteid,$optionid,$stdid,2,'exam_mark').'</strong></td><td style="color:blue; font-weight:bold;"><strong>'.SubTotal($db,$activiteid,$optionid,$stdid,$i,'max_tri').'</strong></td><td><strong>'.SubTotal($db,$activiteid,$optionid,$stdid,$i,'max_tri_mark_second_tim').'</strong></td>';	
			
		}else
		{
			//OTHER PERIODE THRID TRIM ++
			
			$maxes='<td style="color:blue; font-weight:bold;">'.SubTotal($db,$activiteid,$optionid,$stdid,$i,'max_exam').'</td><td>'.SubTotal($db,$activiteid,$optionid,$stdid,3,'exam_mark').'</td><td style="color:blue; font-weight:bold;"><strong>'.SubTotal($db,$activiteid,$optionid,$stdid,$i,'max_tri').'</strong></td><td><strong>'.SubTotal($db,$activiteid,$optionid,$stdid,$i,'max_tri_mark_third_tim').'</strong></td>';	
		}
			}else if($type =='total_gen')
		 	 {
				 if($i<5)
				{
					...
					$maxes='<td style="color:blue; font-weight:bold;">'.TotalGen($db,$stdid,$optionid,$i,'total_gen_max_exam').'</td><td>'.TotalGen($db,$stdid,$optionid,2,'total_gen_exam_mark').'</td> <td><strong>'.TotalGen($db,$stdid,$optionid,$i,'total_gen_max_tri').'</strong></td><td><strong>'.TotalGen($db,$stdid,$optionid,$i,'max_tri_mark_second_tim').'</strong></td>';	
				}
				else
				{
					//OTHER PERIODE THRID TRIM ++
				$maxes='<td style="color:blue; font-weight:bold;">'.TotalGen($db,$stdid,$optionid,$i,'total_gen_max_exam').'</td><td><strong>'.TotalGen($db,$stdid,$optionid,3,'total_gen_exam_mark').'</strong></td> <td><strong>'.TotalGen($db,$stdid,$optionid,$i,'total_gen_max_tri').'</strong></td><td><strong>'.TotalGen($db,$stdid,$optionid,$i,'max_tri_mark_three_tim').'</strong></td>';	
				}
			 }
			$max_p='';
		}
		//*****************************2EM TRIME STRRE ENDS*********************************************************************************
		else
		{
			$j++;
		  if($j==3)
		  {
			$j=0;
			 ...
			 if($type =='max_header')
		     {
		 	 ...
		 	 {
				 //mark exam
				 $point_exam=foreign_value('where cours_id ="'.$brancheid.'" and order_id=1 and affectation_id =2 and inscription__des__eleves_id ="'.$stdid.'"','tbl__37point_eleve','mark',$db);
				 $point_periode1=foreign_value('where cours_id ="'.$brancheid.'" and order_id=1  and affectation_id=1 and inscription__des__eleves_id ="'.$stdid.'"','tbl__37point_eleve','mark',$db);
				 $point_periode2=foreign_value('where cours_id ="'.$brancheid.'" and order_id=2  and affectation_id=1 and inscription__des__eleves_id ="'.$stdid.'"','tbl__37point_eleve','mark',$db);
				 $maxes='<td style="color:blue; font-weight:bold;">'.foreign_value('where branche_id="'.$brancheid.'"',' tbl__43branches','max__exam',$db).'</td><td>'.$point_exam.'</td> <td style="color:blue; font-weight:bold;">'.(foreign_value('where branche_id="'.$brancheid.'"',' tbl__43branches','max__exam',$db)*2).'</td><td>'.($point_exam+$point_periode1+$point_periode2).'</td>';//premier exam premier trimestre
			 }
			 else if($type =='sub_total')
		  	{
			$maxes='<td style="color:blue; font-weight:bold;">'.SubTotal($db,$activiteid,$optionid,$stdid,$i,'max_exam').'</td><td>'.SubTotal($db,$activiteid,$optionid,$stdid,1,'exam_mark').'</td> <td style="color:blue; font-weight:bold;"><strong>'.SubTotal($db,$activiteid,$optionid,$stdid,$i,'max_tri').'</strong></td><td><strong>'.SubTotal($db,$activiteid,$optionid,$stdid,$i,'max_tri_mark_first_tim').'</strong></td>';//subtotal exam premier trim
		 	 }
			  else if($type =='total_gen')
		 	 {
				$maxes='<td style="color:blue; font-weight:bold;">'.TotalGen($db,$stdid,$optionid,$i,'total_gen_max_exam').'</td><td>'.TotalGen($db,$stdid,$optionid,1,'total_gen_exam_mark').'</td> <td><strong>'.TotalGen($db,$stdid,$optionid,$i,'total_gen_max_tri').'</strong></td><td><strong>'.TotalGen($db,$stdid,$optionid,$i,'max_tri_mark_first_tim').'</strong></td>'; //total gen premier trim
		 	 }
			$max_p='';
		   }	
			else
		...
		//content concanating section starts
		if($type == 'max_header')
		{
	  $content.=$max_p.' <td>'.$i.'eP</td>'.$maxes;	
		}
		else if($type =='mark')
		{
	  $content.=$max_p.' <td>'.foreign_value('where cours_id ="'.$brancheid.'" and order_id="'.$i.'"  and affectation_id=1 and inscription__des__eleves_id ="'.$stdid.'"','tbl__37point_eleve','mark',$db).'</td>'.$maxes;	
		}
		else if($type =='sub_total')
		{
	  $content.=$max_p.' <td><strong>'.SubTotal($db,$activiteid,$optionid,$stdid,$i,'periode_mark').'<strong></td>'.$maxes;	//subtotal point obtenu par periode
		}
		else if($type =='total_gen')
		{
	  $content.=$max_p.' <td><strong>'.TotalGen($db,$stdid,$optionid,$i,'total_gen_periode_mark').'<strong></td>'.$maxes;	//subtotal point obtenu par periode
		}
	  //content concanating section ends
	}
	
	if($type =='max_header')
	{
	return $header.$content.$max_header.$footer;
	}
	else if($type =='mark')
	{
		return $content.'<td style="color:blue; font-weight:bold;">'.(foreign_value('where branche_id="'.$brancheid.'"',' tbl__43branches','max__trimestre',$db)*3).'</td><td>'.sum_rowsone($db,'tbl__37point_eleve','mark','where cours_id ="'.$brancheid.'" and order_id<7  and affectation_id<3 and inscription__des__eleves_id ="'.$stdid.'"').'</td>';
	}
	else if($type =='sub_total')
	{
		return $content.'<td style="color:blue; font-weight:bold;">'.SubTotal($db,$activiteid,$optionid,$stdid,$i,'full_max').'</td><td>'.SubTotal($db,$activiteid,$optionid,$stdid,$i,'full_max_mark').'</td>';
	}
	else if($type =='total_gen')
	{
		return $content.'<td style="color:blue; font-weight:bold;">'.TotalGen($db,$stdid,$optionid,$i,'full_max').'</td><td><strong>'.TotalGen($db,$stdid,$optionid,$i,'full_max_mark').'<strong></td>';
	}
	
	
}
function ShowActivities($db,$optionid,$stdid)
{
	$content='';
	$q=$db=mysql_query("select * from  tbl__42activites where option_id='".$optionid."'") or die('Query Failed'.mysql_error());
	while($row=mysql_fetch_array($q))
	{
		$content.=' <tr bgcolor="#C1DFFD" style="text-align:center; text-transform:uppercase;"><td colspan="22">'.$row['activite'].'</td></tr>'.ShowBrancheDip($db,$row[0],$optionid,'',$stdid);
	}
	return $content;
	
}
function ShowBrancheDip($db,$activiteid,$optionid,$type,$stdid)
{
	$content='';
	$q=$db=mysql_query("select * from tbl__43branches where activite_id ='".$activiteid."' and option_id='".$optionid."'") or die('Query Failed'.mysql_error());
	while($row=mysql_fetch_array($q))
	{
		$content.='<tr><td>'.$row['branche'].'</td>'. ShowPeriodeAndmax($db,$optionid,'mark',$row[0],$stdid,$activiteid).'</tr>';
	}
	$subtotal='<tr bgcolor="#FFFFCC"><td>SOUS-TOTAL</td>'.ShowPeriodeAndmax($db,$optionid,'sub_total',$row[0],$stdid,$activiteid).'</tr>';
	return $content.$subtotal;
}
function SubTotal($db,$activiteid,$optionid,$stdid,$i,$type)
{
	$total=0;
	...
	while($row=mysql_fetch_array($q))
	{
		...
		if($type =='max_periode')
		{
		$total+=+$row['max__periode'];
		}
		else if($type =='periode_mark')
		{
			$total+=+foreign_value('where cours_id ="'.$row[0].'" and order_id="'.$i.'" and affectation_id =1 and inscription__des__eleves_id ="'.$stdid.'"','tbl__37point_eleve','mark',$db);
		}
		else if($type =='max_exam')
		{
			$total+=+$row['max__exam'];
		}
		else if($type =='exam_mark')
		{
			$total+=+foreign_value('where cours_id ="'.$row[0].'" and order_id="'.$i.'" and affectation_id =2	 and inscription__des__eleves_id ="'.$stdid.'"','tbl__37point_eleve','mark',$db);
		}
		else if($type =='max_tri')
		{
			$total+=+($row['max__exam']*2);
		}
		else if($type =='max_tri_mark_first_tim')
		{
			$total+=+ (sum_rowsone($db,'tbl__37point_eleve','mark','where cours_id ="'.$row[0].'" and order_id<3 and affectation_id=1 and inscription__des__eleves_id ="'.$stdid.'"')+sum_rowsone($db,'tbl__37point_eleve','mark','where cours_id ="'.$row[0].'" and order_id=1 and affectation_id=2 and inscription__des__eleves_id ="'.$stdid.'"'));
		}
		else if($type =='max_tri_mark_second_tim')
		{
			$total+=+ (sum_rowsone($db,'tbl__37point_eleve','mark','where cours_id ="'.$row[0].'" and order_id>2 and order_id<5 and affectation_id=1 and inscription__des__eleves_id ="'.$stdid.'"')+sum_rowsone($db,'tbl__37point_eleve','mark','where cours_id ="'.$row[0].'" and order_id=2 and affectation_id=2 and inscription__des__eleves_id ="'.$stdid.'"'));
		}
		else if($type =='max_tri_mark_second_tim')
		{
			$total+=+ (sum_rowsone($db,'tbl__37point_eleve','mark','where cours_id ="'.$row[0].'" and order_id>2 and order_id<5 and affectation_id=1 and inscription__des__eleves_id ="'.$stdid.'"')+sum_rowsone($db,'tbl__37point_eleve','mark','where cours_id ="'.$row[0].'" and order_id=2 and affectation_id=2 and inscription__des__eleves_id ="'.$stdid.'"'));
		}
		else if($type =='max_tri_mark_third_tim')
		{
			$total+=+ (sum_rowsone($db,'tbl__37point_eleve','mark','where cours_id ="'.$row[0].'" and order_id>4 and order_id<7 and affectation_id=1 and inscription__des__eleves_id ="'.$stdid.'"')+sum_rowsone($db,'tbl__37point_eleve','mark','where cours_id ="'.$row[0].'" and order_id=3 and affectation_id=2 and inscription__des__eleves_id ="'.$stdid.'"'));
		}
		else if($type =='full_max')
		{
			$total+=+ ($row['max__trimestre']*3);
		}
		else if($type == 'full_max_mark')
		{
			$total+=+sum_rowsone($db,'tbl__37point_eleve','mark','where cours_id ="'.$row[0].'" and  order_id<7 and affectation_id<3 and inscription__des__eleves_id ="'.$stdid.'"');
		}
...
function TotalGen($db,$stdid,$optionid,$i,$type)
{
	if($type == 'total_gen_max_periode')
	{
		$total_gen=sum_rowsone($db,'tbl__43branches','max__periode','where option_id="'.$optionid.'"');
	}
	if($type == 'total_gen_periode_mark')
	{
		$total_gen=sum_rowsone($db,'tbl__37point_eleve','mark','where inscription__des__eleves_id 	="'.$stdid.'" and order_id="'.$i.'" and affectation_id 	=1');
	}
	if($type == 'total_gen_max_exam')
	{
		$total_gen=sum_rowsone($db,'tbl__43branches','max__exam','where option_id="'.$optionid.'"');
	}
	if($type == 'total_gen_exam_mark')
	{
		$total_gen=sum_rowsone($db,'tbl__37point_eleve','mark','where inscription__des__eleves_id 	="'.$stdid.'" and order_id="'.$i.'" and affectation_id =2');
	}
	if($type == 'total_gen_max_tri')
	{
		$total_gen=(sum_rowsone($db,'tbl__43branches','max__exam','where option_id="'.$optionid.'"')*2);
	}
	else if($type =='max_tri_mark_first_tim')
		{
			$total_gen=(sum_rowsone($db,'tbl__37point_eleve','mark','where  order_id<3 and affectation_id=1 and inscription__des__eleves_id ="'.$stdid.'"')+sum_rowsone($db,'tbl__37point_eleve','mark','where  order_id=1 and affectation_id=2 and inscription__des__eleves_id ="'.$stdid.'"'));
		}
	else if($type =='max_tri_mark_second_tim')
		{
			$total_gen=(sum_rowsone($db,'tbl__37point_eleve','mark','where  order_id<5 and order_id>2 and affectation_id=1 and inscription__des__eleves_id ="'.$stdid.'"')+sum_rowsone($db,'tbl__37point_eleve','mark','where  order_id=2 and affectation_id=2 and inscription__des__eleves_id ="'.$stdid.'"'));
		}
		else if($type =='max_tri_mark_three_tim')
		{
			$total_gen=(sum_rowsone($db,'tbl__37point_eleve','mark','where  order_id<7 and order_id>4 and affectation_id=1 and inscription__des__eleves_id ="'.$stdid.'"')+sum_rowsone($db,'tbl__37point_eleve','mark','where  order_id=3 and affectation_id=2 and inscription__des__eleves_id ="'.$stdid.'"'));
		}
		else if($type =='full_max')
		{
			$total_gen=(sum_rowsone($db,'tbl__43branches','max__trimestre','where option_id="'.$optionid.'"')*3);
		}
		
		else if($type =='full_max_mark')
		{
			$total_gen=sum_rowsone($db,'tbl__37point_eleve','mark','where  order_id<7  and affectation_id<3 and inscription__des__eleves_id ="'.$stdid.'"');
		}
...
function ShowActivites($db,$optionid)
{
	...
	while($row=mysql_fetch_array($q))
	{
		$content.='<li style="text-transform:capitalize;">'.$row['activite'].'<a href=""></a> |<a href="activite.php?optionid='.$optionid.'&id='.$row[0].'" rel="facebox">Edit </a>&nbsp;|<a href="branches.php?ActiviteId='.$row[0].'&OptionId='.$optionid.'" rel="facebox">Branches</a>'.ShowBranches($db,$row[0],$optionid).'</li>';
	}
	return $header.$content.$footer;
}
