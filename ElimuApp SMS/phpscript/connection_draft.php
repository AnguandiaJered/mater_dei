...
$db=mysql_select_db(''.DB.'') or die('unknown database'.mysql_error());
...
function journal_caisse_rpt($db,$table,$ref,$from,$to)
{
	...
	{
		$entree=$row['montant'];
		$sortie=sortie($row['date'],$output='montant',$db);
		$solde=($entree-$sortie);
		$total_entree+=+$entree;
		$total_sortie+=+$sortie;
		$total_solde+=+$solde;
		...
		<td>'.french_date($row['date']).'</td>
		<td>'.$row['motif'].'</td>
		<td align="right">'.$row['montant'].'</td>
		<td>'.sortie($row['date'],$output='libelle',$db).'</td>
		<td align="right">'.sortie($row['date'],$output='montant',$db).'</td>
	...
	<td align="right">'.($total_entree+fee_incoming($db,$from,$to,'total')).'</td>
	<td colspan="2" align="right">'.$total_sortie.'</td>
	<td align="right">'.$total_solde.'</td>
	</tr>';
	return $periode.$header.$body.fee_incoming($db,$from,$to).$footer_total.$footer;
}
function sortie($date,$output='montant',$db)
{
$q=$db=mysql_query("select * from tbl__29sortie where date='".$date."'") or die('Query Failed'.mysql_error());	
$total=0;
$content='';
while($read=mysql_fetch_array($q))
{
...
if($output=='montant'){return $total;}else{return $content;}
}

function fee_incoming($db,$from,$to,$type='data')
{
	...
	while($row=mysql_fetch_array($q))
	{
		$fee=foreign_value('where frais__scolaires_id="'.$row['frais__scolaires_id'].'"','tbl__14frais__scolaires','nom__du__frais',$db);
		$student_name=foreign_value('where inscription__des__eleves_id="'.$row['inscription__des__eleves_id'].'"','tbl__17inscription__des__eleves','nom__complet',$db);
		$classe_id=foreign_value('where inscription__des__eleves_id="'.$row['inscription__des__eleves_id'].'"','tbl__17inscription__des__eleves','classe_id',$db);
		$classe=foreign_value('where  classe_id="'.$classe_id.'"',' tbl__15classe','nom__de__la__classe',$db);
		$content.='<tr bgcolor="#FFFFFF">
		<td>'.french_date($row['date']).'</td>
		<td>'.$fee.'/'.$student_name.'/'.$classe.'</td>
		<td align="right">'.$row['montant'].'</td>
		<td></td>
		<td align="right"></td>
		<td align="right"></td>
